...
namespace WindowsServiceCS
{
    ...
    private void SchedularCallback(object sender, System.Timers.ElapsedEventArgs e)
    {
        ...
        try
        {
            var ftpdirectory = new DirectoryInfo(@"C:\ftp\data\Stock\");
                ...
                foreach (var filename in myFile)
                {
                    ...
                    if (DbFileName == "")
                    {
                        ...
                        using (WebClient ftpClient = new WebClient())
                        {
                            ...
                            {
                                using (SqlCommand spcmd = new SqlCommand("Sp_TallyReceipt", connnn))
                                {
                                    spcmd.CommandType = CommandType.StoredProcedure;

                                    spcmd.Parameters.Add("@Path", SqlDbType.VarChar).Value = @"C:\ftp\data\Stock\" + filename.Name;
        ...
        {
            string path = "C:\\hostinglog\\LogisticStockTestServiceLog.txt";
